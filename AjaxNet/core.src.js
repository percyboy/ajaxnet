var AjaxNet = {
    invoke: function (url, method, args, callback) {
        var op = {
            ...
            beforeSend: function (jqXHR) {
                jqXHR.setRequestHeader("X-AjaxNet-Method", method);
                if (AjaxNet.token && AjaxNet.token.length > 0) {
                    jqXHR.setRequestHeader("X-AjaxNet-Token", AjaxNet.token);
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                this.__r.duration = new Date().getTime() - this.__r.duration;
                if (textStatus == "abort") return;
                this.__r.error = { Message: "Unknown", Type: "ConnectFailure" };
                if (errorThrown) { this.__r.error.Message = errorThrown; }
                else { this.__r.error.Message = textStatus; }
            ...
            success: function (result) {
                ...
                if (result != null) {
                    if (typeof (result.value) != "undefined") this.__r.value = result.value;
                    else if (typeof (result.error) != "undefined") this.__r.error = result.error;
                }
                if (typeof (callback) == "function") {
                    callback(this.__r);
                }
        ...
        op.data = JSON.stringify(args, function (key, value) {
            var value2 = value;
            for (var i = 0; i < AjaxNet.jsonConverters.length; i++) {
                var c = AjaxNet.jsonConverters[i];
        ...
        if (typeof (callback) == "function") {
            ...
            op.async = false;
            var result = jQuery.ajax(op).responseText;
            op.success(result);
        }
        return op.__r;
    },
    ...
    DataRow: function (dt, data) {
        if (jQuery.isArray(data)) this.data = data;
        else this.data = [];
...
AjaxNet.jsonConverters.push({
    fromJSON: function (value, oldValue) {
        ...
        if (typeof (value) == "object" && value != null && jQuery.isArray(value.keys) && jQuery.isArray(value.values))
